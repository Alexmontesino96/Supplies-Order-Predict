...
def import_product_month_csv(product_month_csv: UploadFile = File, user: Auth0User = Security(auth.get_user)) -> (
        list)[Product_In_Schema]:
    """
...
@product_app.get("/products/search_product", tags=["Product"], dependencies=[Depends(auth.implicit_scheme)])
def search_products_by_id(product_id: str, user: Auth0User = Security(auth.get_user)) -> JSONResponse:
    """
    ...
    """
    product = ProductServiceCSV(Session).get_products_by_id(product_id)
    if not product:
...
@product_app.delete("/products/delete_product", tags=["Product"], dependencies=[Depends(auth.implicit_scheme)])
def delete_product(product_id: str, user: Auth0User = Security(auth.get_user)) -> JSONResponse:
    """
    Delete a product.
    ...
    """
    return ProductServiceCSV(Session).delete_product(product_id)


@product_app.put("/products/update_product", tags=["Product"], dependencies=[Depends(auth.implicit_scheme)])
def update_product(product_id: str, product: Product_In_Schema,
                   user: Auth0User = Security(auth.get_user)) -> JSONResponse:
    """
    Update a product.
    ...
    """
    with session_scope() as session:
        return ProductServiceCSV(session).update_product(product_id, product)
